...
import {markets, mints, reserves, getInitReservePdas, getLendingMarketAuthPDA, reserveFarmStates} from "./retrieve_pdas";
...
class Kamino {
    ...
    constructor(program: Program, provider: AnchorProvider, market: string, mint: string){
    ...
    async deposit(amount: BN): Promise<TransactionInstruction[]> {
        ...
        const initReservePdas = getInitReservePdas(this.kaminoLending, this.market, this.mint);
        ...
        const obligationFarmInfo = await this.program.account.obligationFarm.fetchNullable(obligationFarm);
            ...
            instructions.push(
                await this.program.methods.InitObligationFarmsForReserve(0)
                    .accounts({
                        ...
                        obligation: obligation,
                        lendingMarketAuthority: getLendingMarketAuthPDA(this.kaminoLending, this.market),
            ...
            await this.program.methods.depositReserveLiquidityAndObligationCollateralV2(amount)
                .accounts({
                    depositAccounts: {
                        ...
                        reserveLiquiditySupply: initReservePdas.liquiditySupplyVault,
                        reserveCollateralMint: initReservePdas.collateralCTokenMint,
                        reserveDestinationDepositCollateral: initReservePdas.collateralSupplyVault,
    ...
    async withdraw(amount: BN): Promise<TransactionInstruction[]> {
        let instructions: TransactionInstruction[] = [];
        instructions.push(
            await this.program.methods.withdrawReserveLiquidityAndObligationCollateralV2(amount)
                .accounts({
                    withdrawAccounts: {
                        owner: this.provider.wallet.publicKey,
                        obligation: this.obligation(),
                        ...
                        reserve: this.reserve,
                        reserveLiquidityMint: this.mint,
                        reserveLiquiditySupply: getInitReservePdas(this.kaminoLending, this.market, this.mint).liquiditySupplyVault,
                        reserveCollateralMint: getInitReservePdas(this.kaminoLending, this.market, this.mint).collateralCTokenMint,
                        reserveSourceDepositCollateral: getInitReservePdas(this.kaminoLending, this.market, this.mint).collateralSupplyVault,
                        userDestinationLiquidity: getAssociatedTokenAddressSync(this.mint, this.provider.wallet.publicKey),
                        placeholderUserSourceCollateral: this.kaminoLending,
                        collateralTokenProgram: TOKEN_PROGRAM_ID,
                        liquidityTokenProgram: TOKEN_PROGRAM_ID,
                        instructionSysvarAccount: SYSVAR_INSTRUCTIONS_PUBKEY,
                    },
